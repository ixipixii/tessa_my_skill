CREATE PROCEDURE [Sync_11_OrdersUK]
AS
BEGIN
	...
	BEGIN TRY		
		...
		DECLARE @CardType UNIQUEIDENTIFIER = '8d8d1098-3b12-4a77-a988-4278f11d9039' -- PnrOrderUK
		...
		where [GroupTypeTESSA] like N''Приказы УК''
		...
		SET
			...
			--,[proj].[DocumentKindID] = CASE WHEN [obj].[DocTypeTESSA] = N'по обеспечению корпоративной мобильной связи' THEN '42e6e6af-c176-4b37-9490-44d4d256c028'
			--ELSE (SELECT TOP(1) [ID] FROM [PnrDocumentKinds] WHERE LOWER([Name]) = LOWER([obj].[DocTypeTESSA])) END
			--,[proj].[DocumentKindName] = CASE WHEN [obj].[DocTypeTESSA] = N'по обеспечению корпоративной мобильной связи' THEN N'По обеспечению корп.моб.связи'
			--ELSE (SELECT TOP(1) [Name] FROM [PnrDocumentKinds] WHERE LOWER([Name]) = LOWER([obj].[DocTypeTESSA])) END
		FROM #ObjectsToUpdate as [obj] WITH(NOLOCK)
		INNER JOIN [PnrOrderUK] as [proj] WITH(NOLOCK) ON [proj].[ExtID] = [obj].[UniqueId]
		...
		FROM #ObjectsToUpdate as [obj] WITH(NOLOCK)
		INNER JOIN [PnrOrderUK] as [inc] WITH(NOLOCK) ON [inc].[ExtID] = [obj].[UniqueId]
		INNER JOIN [DocumentCommonInfo] as [proj] WITH(NOLOCK) ON  [proj].[ID] = [inc].[ID]
			
		--очистим выгрузку от существующих записей
		DELETE #ObjectsToUpdate WHERE [UniqueId] IN (SELECT [ExtID] from [PnrOrderUK] WHERE [ExtID] IS NOT NULL)
		...
		SELECT 
			...
			--,CASE WHEN [obj].[DocTypeTESSA] = N'по обеспечению корпоративной мобильной связи' THEN '42e6e6af-c176-4b37-9490-44d4d256c028'
			--ELSE (SELECT TOP(1) [ID] FROM [PnrDocumentKinds] WHERE LOWER([Name]) = LOWER([obj].[DocTypeTESSA])) END AS [DocumentKindID]
			--,CASE WHEN [obj].[DocTypeTESSA] = N'по обеспечению корпоративной мобильной связи' THEN N'По обеспечению корп.моб.связи'
			--ELSE (SELECT TOP(1) [Name] FROM [PnrDocumentKinds] WHERE LOWER([Name]) = LOWER([obj].[DocTypeTESSA])) END AS [DocumentKindName]
		...
		SELECT 
			t.[ID]
			,t.[MainCardId]
			,'b9c0c3e7-d257-42e1-ad36-54a3636c22b7'
			,N'Зарегистрирован'
