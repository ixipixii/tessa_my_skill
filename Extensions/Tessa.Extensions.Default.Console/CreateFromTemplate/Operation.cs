        ...
        ConsoleOperation<OperationContext>
    {
        ...
        private async Task<CardStoreResponse> CreateFromTemplateCoreAsync(string fileName, CardFileFormat format, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                {
                    newResponse = await this.cardManager.CreateFromTemplateAsync(
                        fileStream,
                        async p =>
                        {
                            if (container[0] == null)
                            {
                                container[0] = await this.cardFileManager.CreateContainerAsync(p.Card, cancellationToken: cancellationToken);
                            }
