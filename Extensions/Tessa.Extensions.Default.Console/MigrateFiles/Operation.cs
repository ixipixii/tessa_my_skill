...
namespace Tessa.Extensions.Default.Console.MigrateFiles
{
    public static class Operation
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            Func<CancellationToken, Task<DbManager>> createDbManagerFunc = DefaultConsoleHelper.CreateAsyncDbManagerFunc(configurationString, databaseName);

            IUnityContainer container = new UnityContainer()
                    .RegisterServerForConsole(() => createDbManagerFunc(cancellationToken).GetAwaiter().GetResult())
                ;
            ...
            await using (dbScope.Create())
            {
                ...
                var fileSourceSettings = container.Resolve<ICardFileSourceSettings>();

                ICardFileSource fromSource = await fileSourceSettings.TryGetAsync(fromSourceTypeID, cancellationToken);
                if (fromSource == null)
                {
                ...
                ICardFileSource toSource = await fileSourceSettings.TryGetAsync(toSourceTypeID, cancellationToken);
                if (toSource == null)
                {
            ...
            ICardContentStrategy fromSourceContentStrategy = await contentStrategy.GetContentStrategyAsync(fromSourceTypeID, cancellationToken);
            ICardContentStrategy toSourceContentStrategy = await contentStrategy.GetContentStrategyAsync(toSourceTypeID, cancellationToken);
