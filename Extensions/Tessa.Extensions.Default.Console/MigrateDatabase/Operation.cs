...
DEALLOCATE query_cursor;";
        ...
        private static void ResolveConnectionParameters(
            string sourceConfigurationString,
            string sourceDatabaseName,
            out string connectionString,
            out DbProviderFactory dbProviderFactory,
            out ConfigurationDataProvider configurationDataProvider)
        {
            ...
            configurationDataProvider = dataProvider;

            dbProviderFactory = ConfigurationManager
                .GetConfigurationDataProviderFromType(configurationConnection.DataProvider)
                .GetDbProviderFactory();

            connectionString = configurationConnection.ConnectionString;
            ...
            if (!string.IsNullOrEmpty(sourceDatabaseName))
            {
                ...
                {
                    throw new InvalidOperationException($"Connection builder is null in method {nameof(Operation)}.{nameof(ResolveConnectionParameters)}");
                }
        ...
        private static Task MigrateDataAsync(
            ...
            ThreadDbContext[] contextArray,
            List<SchemeTable> tables,
            CancellationToken cancellationToken = default)
        {
            if (contextArray.Length == 0 || tables.Count == 0)
            {
            ...
            {
                ThreadDbContext context = contextArray[threadIndex];
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                DbType[] columnDbTypes = columns.Select(x => x.Type.DbType).ToArray();
                ...
                {
                    // режим массовой вставки bulk insert
                    string queryText = context.InsertExecutor
                        .GetBulkInsertCommandText(
                            context.TargetDb,
                    ...
                    context.TargetDb
                        .SetCommand(queryText, targetCommandParameters)
                        .WithoutTimeout();

                    // этом массивы значений, наполняемые через SELECT на исходной базе
                    var parameterValueArrays = new object[targetCommandParameters.Length][];
                    ...
                    var taskParameterValueArrays = new object[targetCommandParameters.Length][];
                    ...
                    if (row > 0)
                    {
                        ...
                        await context.InsertExecutor.BulkInsertAsync(context.TargetDb, row, columnDbTypes, parameterValueArrays, cancellationToken);
                    }
                    else
                    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            ResolveConnectionParameters(
                ...
                sourceDatabaseName,
                out string sourceConnectionString,
                out DbProviderFactory sourceDbProviderFactory,
                out ConfigurationDataProvider sourceConfigurationDataProvider);

            await logger.InfoAsync(sourceConnectionString);

            var sourceDatabaseSchemeService = new DatabaseSchemeService(() =>
                CreateDbConnection(sourceDbProviderFactory, sourceConnectionString));

            if (!sourceDatabaseSchemeService.IsStorageExists)
            {
                await logger.ErrorAsync("Scheme doesn't exists in the source database");
                return -1;
            }

            if (!sourceDatabaseSchemeService.IsStorageUpToDate)
            {
            ...
            database.Refresh(sourceDatabaseSchemeService);

            SchemeDatabase databaseFinal = new SchemeDatabase(DatabaseNames.Original);
            databaseFinal.Refresh(database);
            ...
            foreach (SchemeTable table in tables)
            {
                if (!table.IsSealed)
                {
                    ...
                    {
                        if (!column.IsSealed
                            && column is SchemeComplexColumn complexColumn
                            && complexColumn.WithForeignKey)
                        {
            ...
            ResolveConnectionParameters(
                ...
                targetDatabaseName,
                out string targetConnectionString,
                out DbProviderFactory targetDbProviderFactory,
                out ConfigurationDataProvider targetConfigurationDataProvider);

            await logger.InfoAsync(targetConnectionString);

            var targetDatabaseSchemeService = new DatabaseSchemeService(() =>
                CreateDbConnection(targetDbProviderFactory, targetConnectionString));

            if (!targetDatabaseSchemeService.IsStorageExists)
            {
                await logger.InfoAsync("Scheme doesn't exists in the target database, creating it");
                targetDatabaseSchemeService.CreateStorage();
            }

            if (!targetDatabaseSchemeService.IsStorageUpToDate)
            {
                await logger.InfoAsync("Scheme isn't up-to-date in the target database, upgrading it");
                targetDatabaseSchemeService.UpdateStorage();
            }

            await logger.InfoAsync("Importing initial scheme to the target database");
            database.SubmitChanges(targetDatabaseSchemeService);
            ...
            var contextArray = new ThreadDbContext[threadCount];
            ...
            try
            {
                for (int threadIndex = 0; threadIndex < contextArray.Length; threadIndex++)
                {
                    ...
                    var context = new ThreadDbContext(sourceDb, sourceBuilderFactory, sourceDbms, targetDb, targetBuilderFactory, targetDbms, insertExecutor, bulkSize);
                ...
                if (contextArray.Length > 0)
                {
                    ...
                    ThreadDbContext context = contextArray[0];
                    ...
                    foreach (ThreadDbContext dbContext in contextArray)
                    {
                        await dbContext.SourceDb.CloseAsync(cancellationToken);
                        await dbContext.TargetDb.CloseAsync(cancellationToken);
                    }
                }
            }
            finally
            {
                ...
                {
                    ThreadDbContext context = contextArray[i];
                    if (context != null)
                    {
                        context.TargetDb.Dispose();
                        context.SourceDb.Dispose();
                    }
                }
            }
            ...
            databaseFinal.SubmitChanges(targetDatabaseSchemeService);
