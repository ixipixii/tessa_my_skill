...
namespace Tessa.Extensions.Default.Console.SqlScript
{
    public static class Operation
    {
        ...
        {
            string[] sourceArray = source as string[] ?? source?.ToArray() ?? EmptyHolder<string>.Array;
            ...
            try
            {
                ...
                async Task<DbManager> ExecuteScriptAsync(DbManager dbParam, string sqlText, CancellationToken ct = default)
                {
                    ...
                    if (dbParam == null)
                    {
                        ...
                        dbParam = await DefaultConsoleHelper.CreateDbManagerAsync(logger, configurationString, databaseName, ct);
                    }

                    if (sqlCommands.Length > 0)
                    {
                        string[] parametersArray = (parameters ?? Enumerable.Empty<string>()).ToArray();

                        var dbParameters = new DataParameter[parametersArray.Length];
                        for (int i = 0; i < parametersArray.Length; i++)
                        {
                            ...
                            string p = (parametersArray[i] ?? string.Empty).Trim();
                            if (p.Length == 0)
                            {
                                continue;
                            }
                            ...
                            if (separator < 0)
                            {
                                ...
                                dbParameters[i] = dbParam.Parameter(p, DBNull.Value, DataType.NVarChar);
                            }
                            else if (separator < p.Length - 1)
                            {
                                // разделитель есть, и значение справа тоже есть
                                dbParameters[i] =
                                    dbParam.Parameter(p.Substring(0, separator), p.Substring(separator + 1), DataType.NVarChar);
                            }
                            else
                            {
                                // разделитель есть, но значения нет, отрезаем его, а в качестве значения указываем пустую строку
                                dbParameters[i] = dbParam.Parameter(p.Substring(0, p.Length - 1), string.Empty, DataType.NVarChar);
                            }
                        }
                        ...
                        for (int i = 0; i < sqlCommands.Length; i++)
                        {
                            ...
                            if (i >= 1)
                            {
                                dbParameters[dbParameters.Length - 1] = dbParam.Parameter("Result", outputResult);
                                ...
                                text = text.Replace("@Result@", FormattingHelper.FormatToString(outputResult) ?? string.Empty);
                            }
