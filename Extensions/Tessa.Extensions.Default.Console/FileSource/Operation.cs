                ...
                else
                {
                    ...
                    if (sourceRow == null)
                    {
                        ...
                        if (hasFileLocation)
                        {
                            ...
                            await this.Logger.InfoAsync(
                                "File source is located in file folder = \"{0}\", default = {1}, legacy format = {2}",
                                context.FileLocation,
                                isDefault,
                                context.UseSimpleNamingScheme);

                            newRow["IsDatabase"] = BooleanBoxes.False;
                            newRow["Path"] = context.FileLocation.Trim();
                            newRow["UseSimpleNamingScheme"] = BooleanBoxes.Box(context.UseSimpleNamingScheme);
                        }
                        else
                        {
                            // база данных
                    ...
                    else
                    {
                        ...
                        if (hasFileLocation)
                        {
                            ...
                            sourceFields["UseSimpleNamingScheme"] = BooleanBoxes.Box(context.UseSimpleNamingScheme);
                        }
                        else
                        {
                            // база данных
                            ...
                            sourceFields["UseSimpleNamingScheme"] = BooleanBoxes.False;
                        }
                    }
