...
namespace Tessa.Extensions.Default.Server.Views
{
    ...
    /// </summary>
    public sealed class ViewsInterceptor : IViewInterceptor
    {
        #region Constants
        ...
        {
            var metadata = (await this.viewServiceFactory().GetByNameAsync(ViewFiles, cancellationToken))?.Metadata;
        ...
        /// </summary>
        public string[] InterceptedViews { get; } = { ViewFiles };
        ...
        private sealed class ViewFilesInterceptor : ITessaView
        {
            ...
            public async Task<ITessaViewResult> GetDataAsync(ITessaViewRequest request, CancellationToken cancellationToken = default)
            {
                var result = this.GetParentFolderItems(request);
                if (result != null)
                {
                    return result;
                }

                result = this.GetFolderItems(request);
                return result ?? this.DefaultFolderResult(request);
            }
            ...
            /// </returns>
            private ITessaViewResult GetResult(IEnumerable<string> files)
            {
                string sqlStringTypeName = this.schemeTypeConverter.TryGetSqlTypeName(SchemeType.String, this.dbScope.Dbms);
            ...
            /// </returns>
            private ITessaViewResult DefaultFolderResult(ITessaViewRequest request)
            {
                var filter = GetDirectoryFilter(request);
                var files = GetItems(DefaultDirectory, filter, false);
                return this.GetResult(files);
            }
            ...
            /// Результат выполнения запроса
            /// </returns>
            private ITessaViewResult GetFolderItems(ITessaViewRequest request)
            {
                ITessaViewResult result = null;
                if (request.IsDefinedParameter(Folder))
                {
                    request.IterateParameterCriterias(
                        Folder,
                        criteria =>
                        {
            ...
            private static IEnumerable<string> GetItems(string path, string filter, bool getFolders)
            {
                if (!Directory.Exists(path))
                {
                    ...
                    return EmptyHolder<string>.Array;
                }
            ...
            /// Результат выполнения запроса
            /// </returns>
            private ITessaViewResult GetParentFolderItems(ITessaViewRequest request)
            {
                ITessaViewResult result = null;
