...
namespace Tessa.Extensions.Default.Server.Files
{
        ...
        CardGetFileContentExtension
    {
        ...
        private const string CreateBarcodeKey = CardHelper.SystemKeyPrefix + "CreateBarcode";
        ...
        {
            Card settingsCard = await cardCache.Cards.GetAsync("DocLoad", context.CancellationToken);
            ...
            await using (this.dbScope.Create())
            {
                ...
                if (!isEnabled || !this.licenseManager.License.Modules.Contains(LicenseModules.DocLoadID))
                {
                    return;
                }

                var barcode = await db.SetCommand(this.dbScope.BuilderFactory
                        .Select()
                        .C(fieldName)
                        ...
                        .C("ID").Equals().P("ID")
                        .Build(),
                    db.Parameter("ID", context.Request.CardID)).ExecuteAsync<string>(context.CancellationToken);
                if (string.IsNullOrWhiteSpace(barcode))
                {
                    ...
                    var storeRequest = new CardStoreRequest { Card = card, Info = { [CreateBarcodeKey] = BooleanBoxes.True } };
                    ...
                    barcode = storeResponse.Info.Get<string>(CreateBarcodeKey);
                    context.Response.Info["RefreshCard"] = true;
                }
                ...
                catch (Exception e)
                {
                    ...
                    throw new Exception("Invalid input barcode. Check barcode format!", e);
                }
            }
        }
