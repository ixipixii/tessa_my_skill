...
namespace Tessa.Extensions.Default.Server.Cards
{
        ...
        PlaceholderDocument
    {
        ...
        protected abstract IEnumerable<IPlaceholderText> GetPlaceholdersFromElementOverride(OpenXmlElement baseElement, List<object> position);
            ...
            if (relationship != null)
            {
                ...
                foreach (IPlaceholderReplacement replacement in replacements)
                {
                    ...
                    if (WithExtensions)
                    {
                        ...
                        ((OpenXmlPlaceholderReplaceExtensionContext)ExtensionContext).PlaceholderText = newValue.Text;
                        await AfterPlaceholderReplaceAsync(ExtensionContext.ReplacementContext);
                        newText = ((OpenXmlPlaceholderReplaceExtensionContext)ExtensionContext).PlaceholderText;
                    }
                    else
                    {
                ...
                mainPart.AddHyperlinkRelationship(new Uri(Uri.EscapeUriString(sb.ToString()), UriKind.RelativeOrAbsolute), true, relID);
            }
        }
        ...
        protected async Task<string> ReplaceElementsInRelationshipsWithCopyAsync(
            ...
            params IPlaceholderReplacement[] replacements)
        {
            string origRelID = (string)replacements[0].Placeholder.Info.Get<List<object>>(OpenXmlHelper.PositionField).Last();
            ReferenceRelationship relationship = mainPart.HyperlinkRelationships.FirstOrDefault(a => a.Id == relID);
            if (relationship != null)
            {
                StringBuilder sb = new StringBuilder(Uri.UnescapeDataString(relationship.Uri.ToString()));
                ...
                if (!string.Equals(relID, origRelID, StringComparison.CurrentCultureIgnoreCase))
                {
                    mainPart.DeleteReferenceRelationship(relationship);
                }
                ReferenceRelationship newRelationsip = mainPart.AddHyperlinkRelationship(new Uri(Uri.EscapeUriString(sb.ToString()), UriKind.RelativeOrAbsolute), true);
            ...
            params IPlaceholderReplacement[] placeholders)
        {
            ...
            if (WithExtensions)
            {
                ExtensionContext.Placeholder = curPlaceholder.Placeholder;
                ExtensionContext.PlaceholderValue = curPlaceholder.NewValue;
                await BeforePlaceholderReplaceAsync(ExtensionContext.ReplacementContext);
                if (!curPlaceholder.NewValue.Equals(this.ExtensionContext.PlaceholderValue))
                {
                    curPlaceholder = new PlaceholderReplacement(curPlaceholder.Placeholder, ExtensionContext.PlaceholderValue);
                }
            }
            ...
            foreach (OpenXmlElement run in element.ChildElements.ToList())
            {
                ...
                do
                {
                    res = await ReplaceElementAsync(run, curPlaceholder, cancellationToken, placeholderElements.ToArray());
            ...
            params OpenXmlElement[] partElements)
        {
            ...
            if (baseElementText == null
                || !CheckTextElement(baseElementText))
            {
                return fromStart
            ...
            if (baseElementText.Text.Contains(replacement.Placeholder.Text))
            {
                await this.ReplaceTextAsync(
                    baseElement, baseElementText, null, null,
                    baseElementText.Text.Replace(replacement.Placeholder.Text, newValue.Text), cancellationToken);
            ...
            if (fromStart
                && baseElementText.Text.Contains(PlaceholderHelper.LeftBracket)
            ...
            if (!fromStart && baseElementText.InnerXml.Contains(PlaceholderHelper.RightBracket))
            {
        ...
        {
            List<IPlaceholderText> result = GetPlaceholdersFromElement(basePart.RootElement, position);
            ...
            foreach (IdPartPair part in basePart.Parts)
            {
                ...
                result.AddRange(GetPlaceholdersFromPart(part.OpenXmlPart, newPosition.ToArray()));
            }
        ...
        protected List<IPlaceholderText> GetPlaceholdersFromElement(OpenXmlElement baseElement, params object[] position)
        {
            ...
            foreach (OpenXmlElement element in baseElement.ChildElements)
            {
                ...
                result.AddRange(GetPlaceholdersFromElementOverride(element, newPosition));

                if (element.HasChildren)
                {
                    result.AddRange(GetPlaceholdersFromElement(element, newPosition.ToArray()));
                }
            }
        ...
        protected static OpenXmlElement GetElementByPlaceholder(OpenXmlPart mainPart, IPlaceholder placeholder)
        {
            ...
            List<object> position = placeholder.Info.Get<List<object>>(OpenXmlHelper.PositionField);
            return GetElementByPosition(mainPart, position);
        }
        ...
        protected static OpenXmlElement GetElementByPosition(OpenXmlPart mainPart, IEnumerable<object> position)
        {
        ...
        protected static void ClearOldRelationships(OpenXmlPart mainPart, IList<IPlaceholder> relationshipsPlaceholders)
        {
            foreach (string relID
                in relationshipsPlaceholders
                    .Where(x => x.Info.Get<OpenXmlElement>(OpenXmlHelper.BaseElementField) != null)
                    .Select(x => x.Info.Get<List<object>>(OpenXmlHelper.PositionField).Last())
                    .Distinct())
            {
        ...
        protected static string TextPosition(IEnumerable<object> position)
        {
        ...
        protected static bool IsRelationship(IPlaceholder placeholder)
        {
            ...
            if (placeholder == null
                || (position = placeholder.Info.TryGet<List<object>>(OpenXmlHelper.PositionField, null)) == null
                )
            {
                return false;
            }
            return position.Count > 0 && position.Any(x => x.ToString() == Hyperlink);
        }
        ...
        protected override async Task<ICollection<IPlaceholderText>> FindCoreAsync(IPlaceholderFindingContext context)
        {
            List<IPlaceholderText> placeholders;
            ...
            if (dbScope != null)
            {
                placeholders = await GetPlaceholdersFromDatabaseAsync(dbScope, context.CancellationToken);
            ...
            using (InitDocument())
            {
                placeholders = GetPlaceholdersFromDocument();
            ...
            if (dbScope != null)
            {
                await SavePlaceholdersInDatabaseAsync(dbScope, placeholders, context.CancellationToken);
            }
        ...
        protected override async Task ReplaceCoreAsync(IPlaceholderReplacementContext context)
        {
            bool hasChanges;
            ...
            using (InitDocument())
            {
                ...
                {
                    await BeforeDocumentReplaceAsync(context);
                }

                bool hasFieldChanges = await ReplaceFieldPlaceholdersAsync(context);
                bool hasTableChanges = await ReplaceTablePlaceholdersAsync(context);
                ...
                if (this.WithExtensions)
                {
                    await AfterDocumentReplaceAsync(context);
                }
