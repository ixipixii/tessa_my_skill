    ...
    public sealed class DocLoadBarcodeStoreExtension : CardStoreExtension
    {
        ...
        public override async Task BeforeCommitTransaction(ICardStoreExtensionContext context)
        {
            ...
            if (!context.ValidationResult.IsSuccessful()
                ...
                || !this.licenseManager.License.Modules.Contains(LicenseModules.DocLoadID)
            ...
            try
            {
                settingsCard = await this.cardCache.Cards.GetAsync("DocLoad", context.CancellationToken);
            }
            ...
            await db.SetCommand(
                context.DbScope.BuilderFactory.
                    Update(tableName).C(fieldName).Assign().P("Value").
                    Where().C("ID").Equals().P("ID").
                    Build(),
