...
namespace Tessa.Extensions.Default.Server.Cards
{
    ...
    public sealed class SaveFileTemplateOnCardStoreExtension : CardStoreExtension
    {
        ...
        {
            if (context.Request.ServiceType != CardServiceType.WebClient ||
        ...
        public override async Task BeforeCommitTransaction(ICardStoreExtensionContext context)
        {
            ...
            using var container = await this.fileManager.CreateContainerAsync(card, cancellationToken: context.CancellationToken);
            var contentRequest =
                new CardGetFileContentRequest
                {
                    ServiceType = CardServiceType.WebClient,
            ...
            if (fileTemplateData.ReplacedFileID == Guid.Empty)
            {
                ...
                storeResponse = await container.StoreAsync(
                    (c, request, ct) =>
                    {
                        request.ServiceType = CardServiceType.WebClient;
                        return Task.CompletedTask;
                    },
            ...
            else
            {
                ...
                if (resp.IsValid())
                {
                    ...
                    {
                        using var cardContainer = await this.fileManager.CreateContainerAsync(cardWithFiles, cancellationToken: context.CancellationToken);
                        ...
                        if (file != null)
                        {
                            ...
                            using (var tempFile = TempFile.Acquire(fileName))
                            {
                                ...
                                {
                                    await using var fileStream = System.IO.File.Create(tempFile.Path);
