        ...
        CardGetExtension
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            db
                .SetCommand(builderFactory
                    .Select().C("t", "TaskRowID").Count()
                    .From("WfTaskCards", "t").NoLock()
                    .InnerJoin("Files", "f").NoLock()
                        .On().C("f", "ID").Equals().C("t", "ID")
                    .Where().C("t", "MainCardID").Equals().P("CardID")
                    .GroupBy("t", "TaskRowID")
                    .Build(),
        ...
        private static Guid? TryGetMainCardIDFromTaskCard(Card card)
        {
            ...
            return sections != null
                && sections.TryGetValue(WfHelper.TaskSatelliteSection, out var section)
                    ? section.RawFields.TryGet<Guid?>(WfHelper.TaskSatelliteMainCardIDField)
                    : null;
        }
                    ...
                    foreach (CardTask task in tasks)
                    {
                        if (WfHelper.TaskTypeIsResolution(task.TypeID))
                        {
                            ...
                            else
                            {
                                ...
                                fields[WfHelper.ResolutionVirtualDigestField] = LocalizationManager.Format(task.Digest);
                                fields[WfHelper.ResolutionVirtualRoleIDField] = task.RoleID;
                                fields[WfHelper.ResolutionVirtualRoleNameField] = task.RoleName;
                            }
                        }
                    }
                }
