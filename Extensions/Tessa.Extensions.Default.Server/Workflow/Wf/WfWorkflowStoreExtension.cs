...
namespace Tessa.Extensions.Default.Server.Workflow.Wf
{
        ...
        WorkflowStoreExtension
    {
        ...
        public WfWorkflowStoreExtension(
            ...
            [Dependency(CardRepositoryNames.ExtendedWithoutTransaction)] ICardRepository cardRepositoryToDeleteSatellite,
            [Dependency(CardRepositoryNames.ExtendedWithoutTransaction)] ICardRepository cardRepositoryToCreateAndStoreSatellite,
        ...
        {
            if (this.historyItemsToDeleteRowIDList == null)
            {
        ...
        protected virtual async ValueTask<bool> CanStartResolutionProcessAsync(ICardStoreExtensionContext context)
        {
            ...
            {
                Guid? docTypeID = KrProcessSharedHelper.GetDocTypeID(mainCardID, dbScope);

                if (!KrComponentsHelper.CheckKrComponents(
                    mainCardID,
                    context.CardType.ID,
                    docTypeID,
                    dbScope,
            ...
            if (this.historyItemsToDeleteRowIDList != null)
            {
                ...
                foreach (Guid rowID in this.historyItemsToDeleteRowIDList)
                {
                    ...
                    if (deletedCount > 0)
                    {
                        ...
                        Guid? taskCardID = await WfHelper.TryGetTaskSatelliteIDAsync(context.DbScope, rowID, context.CancellationToken);
            ...
            CardStoreRequest nextRequest)
        {
            ...
            Guid? satelliteID = await WfHelper.TryGetSatelliteIDAsync(context.DbScope, mainCardID, context.CancellationToken);
            ...
            if (krToken != null)
            {
                ...
                KrToken nextKrToken = this.krTokenProvider.CreateToken(nextCard, krToken.PermissionsVersion, krToken.Permissions);
