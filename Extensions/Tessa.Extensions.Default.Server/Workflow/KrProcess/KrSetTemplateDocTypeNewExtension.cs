        ...
        CardNewExtension
    {
        ...
        public override async Task AfterRequest(ICardNewExtensionContext context)
        {
            ...
            if (!context.RequestIsSuccessful
                ...
                || KrComponentsHelper.GetKrComponents(typeInTemplate.ID, this.typesCache).HasNot(KrComponents.DocTypes))
            {
                return;
            }

            Card cardInTemplate = new Card(templatesFields.Get<byte[]>("Card").ToSerializable().GetStorage());
