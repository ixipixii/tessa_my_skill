    ...
    public sealed class KrReassignAdditionalApprovalStoreExtension : CardStoreExtension
    {
        ...
        public override async Task BeforeCommitTransaction(ICardStoreExtensionContext context)
        {
            ...
            foreach (var task in context.Request.Card.Tasks)
            {
                ...
                var reassingTo = context.Request.Info.TryGet<Guid?>(ReassignTo);
                if (reassingTo == null)
                {
                    continue;
                }
                    ...
                    await db
                        .SetCommand(
                            context.DbScope.BuilderFactory
                                .Update("Tasks").C("ParentID").Assign().P("NewParentID")
                                .Where().C("ParentID").Equals().P("OldParentID")
                                .And().C("TypeID").Equals().P("TaskTypeID").Z()
                                .Update("TaskHistory").C("ParentRowID").Assign().P("NewParentID")
                                .Where().C("ParentRowID").Equals().P("OldParentID")
