...
{
    public sealed class PnrHistoryGetFileContentExtensionShort : CardGetFileContentExtension
    {
        private readonly ICardRepository cardRepository;
        private readonly IDbScope dbScope;

        public static readonly Guid FileHistory = PnrFiles.VirtualFileHistoryShort;
        public static readonly Guid VersionFileHistory = PnrFiles.VersionVirtualFileHistoryShort;

        public PnrHistoryGetFileContentExtensionShort(
            IDbScope dbScope,
        ...
        private System.Data.DataTable GetProcessStages(ICardGetFileContentExtensionContext context, PnrProcess pnrProcess)
        {
            ...
            using (context.DbScope.Create())
            {
                ...
                processStages.Load(db.SetCommand(@"                  
                    SELECT
                    Сотрудник, [Подразделение и должность], [Тип задания], Решение, Комментарий,[Дата получения],[Дата завершения]
                    FROM(

                    SELECT *,
                    ROW_NUMBER() OVER (PARTITION BY Сотрудник ORDER BY Created desc) RN	
                    FROM(

                    SELECT

                    MAX(created) as Created,
                    MAX(Решение) as [Решение],
                    Max(Сотрудник) as [Сотрудник],
	                    MAX([Подразделение и должность]) as [Подразделение и должность],
	                    MaX(Комментарий) as Комментарий,
	                    MAX([Тип задания]) as [Тип задания],
	                    MAX([Дата получения]) as [Дата получения],
	                    MAX([Дата завершения]) as [Дата завершения],
                    role,
                    SubRez
                    ...
                    SELECT 
	                    Сотрудник,
	                    Решение,
	                    Departments + ' - ' +  [должность] as [Подразделение и должность],
	                    [Тип задания],
	                    Комментарий,
	                    [Дата получения],
	                    [Дата завершения],
                    ...
                    SELECT
	                    ...
	                    CAST(
		                    CASE 
		                    WHEN dth.RoleName is NULL or th.OptionCaption  in ('Согласовать','Проверено')
		                    THEN  (SELECT * FROM Localization(th.OptionCaption, 25)) 
		                    ELSE  (SELECT * FROM Localization(th.OptionCaption, 25)) + ' - ' + dth.RoleName
		                    END AS nvarchar(128)) as 'Решение',

		                    pr.Position as 'должность',
		                    --th.RoleName AS 'Роль',
		                    th.RoleID as 'role',
		                    --ISNULL(r.Name, rmain.Name) as 'fake role',
		                    --ISNULL(fth.RoleName, th.RoleName) as 'fake role',
		                    dth.RoleName as SubRoleName,
		                    fsi.BasedOnStageTemplateName AS 'Тип задания',
		                    (SELECT * FROM Localization(th.OptionCaption, 25)) AS 'SubRez',
		                    ...
		                    FROM [RoleUsers] AS [ru] WITH (NOLOCK)
			                    ...
			                    ON [r].[ID] = [ru].[ID]
			                    AND [r].[TypeID] = 2
			                    WHERE [ru].[UserID] = th.UserID
			                    ORDER BY [r].[Name]
			                    FOR XML PATH, TYPE
		                    ).value(N'.[1]', N'nvarchar(max)'), 1, 2, N'') AS [Departments] --подразделение
							 		                    
                    FROM TaskHistory th
	                    JOIN FdCompletedTasks fct ON th.RowID = fct.TaskID -- fd завершенные задачи
	                    LEFT JOIN FdProcessInstances fdi ON fdi.RowID = fct.ProcessRowID  -- процесс fd завершенной задачи
	                    LEFT JOIN FdStageInstances fsi ON fsi.RowID = fct.StageRowID -- этап fd завершенной задачи
	                    Left JOIN TaskHistory fth ON fth.RowId = th.ParentRowID -- определение роли для сотрудника которому делегировали(используется для группировки)
	                    Left JOIN TaskHistory dth ON dth.ParentRowID = th.RowID -- определение роли 
	                    LEFT JOIN Roles r ON r.Id = fth.RoleID
	                    LEFT JOIN Roles rmain ON rmain.Id = th.RoleID
	                    LEFT JOIN PersonalRoles pr ON pr.Id = th.UserID
	                    WHERE fsi.id = @ProccessID
                               AND fdi.BasedOnProcessTemplateID = @ProcessTemplateID) hs

	                    WHERE hs.SubRez  in ('Согласовать', 'Делегировать','Проверено','Делегировать (список)')
	                    ) t
	
	                    Group by role, SubRez)t2)t3
	                    WHERE t3.RN = 1
	                    ORDER BY t3.Created
                    ",
