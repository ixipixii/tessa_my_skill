...
{
    public sealed class PnrUKSuppAgrStoreExtension : CardStoreExtension
    {
        public override async Task BeforeRequest(ICardStoreExtensionContext context)
        {
            ...
            var startDate = await PnrDataHelper.GetActualFieldValueAsync<DateTime?>(context.DbScope, card, "PnrSupplementaryAgreementsUK", "StartDate");
            var endDate = await PnrDataHelper.GetActualFieldValueAsync<DateTime?>(context.DbScope, card, "PnrSupplementaryAgreementsUK", "EndDate");
            var validationResult = context.ValidationResult;

            if (startDate > endDate)
            {
                context.ValidationResult.AddError(this, "Дата начала дополнительного соглашения превышает дату окончания");
            }

            var amount = await PnrDataHelper.GetActualFieldValueAsync<decimal?>(context.DbScope, card, "PnrSupplementaryAgreementsUK", "Amount");
            PnrCardFieldValidationHelper.Validate(validationResult, amount, "Сумма");
        }


        private async Task ValidateFieldsAsync(ICardStoreExtensionContext context, Card card)
        {
            StringDictionaryStorage<CardSection> sections;
            CardSection pnrSupplementaryAgreementsUK = null,
                documentCommonInfo = null;
            if (context.CardType == null
                || (sections = card.TryGetSections()) == null
                || (!sections.TryGetValue("PnrSupplementaryAgreementsUK", out pnrSupplementaryAgreementsUK)
        ...
        public override async Task BeforeCommitTransaction(ICardStoreExtensionContext context)
        {
            ...
            {
                // валидация полей
                await ValidateFieldsAsync(context, card);
            }
        }
    }
}
