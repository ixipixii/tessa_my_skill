            ...
            where T : IStageTypeFormatter
        {
            if (!this.unityContainer.IsRegistered<T>())
            {
                throw new ArgumentException(
                    ...
                    $"Add container.RegisterType<{typeof(IStageTypeFormatter).Name}, {typeof(T).Name}>() in your Registrator class.");
            }
            ...
            Type formatterType)
        {
            if (!this.unityContainer.IsRegistered(formatterType))
            {
                throw new ArgumentException(
                    ...
                    $"Add container.RegisterType<{typeof(IStageTypeFormatter).Name}, {formatterType.Name}>() in your Registrator class.");
            }
