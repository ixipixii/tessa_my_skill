...
namespace Tessa.Extensions.Default.Shared.Workflow.Wf
{
    ...
    public static class WfHelper
    {
        ...
        /// <summary>
        /// Название поля "идентификатор исполнителя" для секции <see cref="ResolutionVirtualSection"/> в задании резолюции.
        /// </summary>
        public const string ResolutionVirtualRoleIDField = "RoleID";

        /// <summary>
        /// Название поля "имя исполнителя" для секции <see cref="ResolutionVirtualSection"/> в задании резолюции.
        /// </summary>
        public const string ResolutionVirtualRoleNameField = "RoleName";
        ...
        /// <summary>
        /// Название поле с идентификатором основной карточки, для задания которой создан сателлит.
        /// Поле содержится в секции <see cref="TaskSatelliteSection"/>.
        /// </summary>
        public const string TaskSatelliteMainCardIDField = "MainCardID";

        /// <summary>
        /// Название поле с идентификатором задания, для которого создан сателлит.
        /// Поле содержится в секции <see cref="TaskSatelliteSection"/>.
        /// </summary>
        public const string TaskSatelliteTaskRowIDField = "TaskRowID";
        ...
        /// по которым можно определить возможность использования резолюций и других бизнес-процессов Workflow.
        /// </returns>
        public static KrComponents GetUsedComponents(IKrTypesCache krTypesCache, Card card)
        {
            ...
            return KrComponentsHelper.GetKrComponents(card, krTypesCache);
        }
        ...
        public static void SetSatelliteMainCardID(Card satellite, Guid mainCardID)
        {
            ...
            satellite.Sections.GetOrAdd(SatelliteSection).RawFields["MainCardID"] = mainCardID;
        }
        ...
        {
            return CardSatelliteHelper.TryGetSatelliteIDAsync(
                dbScope,
                mainCardID,
                SatelliteSection,
                CardSatelliteHelper.MainCardIDColumn,
                cancellationToken);
        }
        ...
        public static WfData TryGetData(Card satellite)
        {
            ...
            if (sections is null
                ...
                || !section.RawFields.TryGetValue("Data", out object data)
                || !(data is byte[] dataBytes))
            {
                return null;
            }
            ...
            bool notifyFieldChanged = true)
        {
            ...
            fields[SatelliteDataField] = data?.ToSerializable().Serialize();
        }
        ...
        /// <summary>
        /// Возвращает список карточек-сателлитов для типовых задач, который был установлен в пакете основной карточки,
        /// или <c>null</c>, если карточки-сателлиты не установлены, т.е. список пустой.
        /// </summary>
        /// <param name="mainCard">Пакет основной карточки, в которой может быть установлен список карточек-сателлитов.</param>
        /// <returns>
        /// Список карточек-сателлитов для типовых задач, который был установлен в пакете основной карточки,
        /// или <c>null</c>, если карточки-сателлиты не установлены, т.е. список пустой.
        /// </returns>
        public static List<Card> TryGetTaskSatelliteList(Card mainCard)
        {
            return CardSatelliteHelper.TryGetSatelliteCardList(mainCard, TaskSatelliteListKey);
        }
        ...
        /// </summary>
        /// <param name="mainCard">Пакет основной карточки, в которой содержится список карточек-сателлитов.</param>
        /// <param name="satellite">
        /// Карточка-сателлит, которая добавляется в список. При этом не выполняется автоматического клонирования структуры.
        /// Может быть равна <c>null</c>, тогда карточек не добавляется, но список карточек-сателлитов будет создан пустым, если он отсутствовал,
        /// что можно использовать как индикация того, что расширение выполнялось, но сателлиты отсутствуют.
        /// </param>
        public static void AddTaskSatelliteToList(Card mainCard, Card satellite)
        {
            CardSatelliteHelper.AddSatelliteToList(mainCard, satellite, TaskSatelliteListKey);
        }
        ...
        /// </summary>
        /// <param name="mainCard">Пакет основной карточки, в которой может содержаться список карточек-сателлитов.</param>
        public static void ClearTaskSatelliteList(Card mainCard)
        {
            CardSatelliteHelper.ClearSatelliteList(mainCard, TaskSatelliteListKey);
        }


        /// <summary>
        /// Создаёт объект <see cref="SatelliteInfo"/>, содержащий информацию по карточке-сателлиту для задания.
        /// </summary>
        /// <param name="satelliteCard">Карточка-сателлит для задания, для которой требуется создать объект.</param>
        /// <returns>Объект <see cref="SatelliteInfo"/>, содержащий информацию по карточке-сателлиту для задания.</returns>
        public static SatelliteInfo CreateSatelliteInfo(Card satelliteCard)
        {
            return CardSatelliteHelper.CreateSatelliteInfo(satelliteCard, TaskSatelliteSection, TaskSatelliteTaskRowIDField);
        }


        /// <summary>
        /// Возвращает список объектов <see cref="SatelliteInfo"/>, содержащий информацию по карточкам-сателлитам задач
        /// для заданной основной карточки, или <c>null</c>, если такие список пустой.
        /// </summary>
        /// <param name="dbScope">Объект, предоставляющий доступ к базе данных.</param>
        /// <param name="mainCardID">Идентификатор основной карточки.</param>
        /// <param name="satelliteTypeID">ID типа сателлита</param>
        /// <param name="cancellationToken">Объект, посредством которого можно отменить асинхронную задачу.</param>
        /// <returns>
        /// Список объектов <see cref="SatelliteInfo"/>, содержащий информацию по карточкам-сателлитам задач
        /// для заданной основной карточки, или <c>null</c>, если такие список пустой.
        /// </returns>
        public static Task<List<SatelliteInfo>> TryGetSatelliteInfoListAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            return CardSatelliteHelper.TryGetSatelliteInfoListAsync(
                dbScope,
                mainCardID,
                satelliteTypeID,
                TaskSatelliteSection,
                TaskSatelliteMainCardIDField,
                cancellationToken);
        }
        ...
        /// или <c>null</c>, если список пустой.
        /// </summary>
        /// <param name="dbScope">Объект, предоставляющий доступ к базе данных.</param>
        /// <param name="mainCardID">Идентификатор основной карточки.</param>
        /// <param name="cancellationToken">Объект, посредством которого можно отменить асинхронную задачу.</param>
        /// <returns>
        /// Список идентификаторов карточек-сателлитов для заданной основной карточки
        /// или <c>null</c>, если список пустой.
        /// </returns>
        public static Task<List<Guid>> TryGetSatelliteIDListAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            return CardSatelliteHelper.TryGetSatelliteIDListAsync(
                dbScope,
                mainCardID,
                TaskSatelliteSection,
                TaskSatelliteMainCardIDField,
        ...
        /// или <c>null</c>, если для задания отсутствует сателлит.
        /// </summary>
        /// <param name="dbScope">Объект, предоставляющий доступ к базе данных.</param>
        /// <param name="taskRowID">Идентификатор задания.</param>
        /// <param name="cancellationToken">Объект, посредством которого можно отменить асинхронную задачу.</param>
        /// <returns>
        /// Идентификатор карточки-сателлита для задания с заданным идентификатором,
        /// или <c>null</c>, если для задания отсутствует сателлит.
        /// </returns>
        public static Task<Guid?> TryGetTaskSatelliteIDAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            return CardSatelliteHelper.TryGetTaskSatelliteIDAsync(
                dbScope,
                taskRowID,
                TaskSatelliteSection,
                TaskSatelliteTaskRowIDField,
                cancellationToken);
        }


        /// <summary>
        /// Возвращает идентификатор основной карточки по идентификатору карточки-сателлита задачи
        /// или <c>null</c>, если сателлит не найден.
        /// </summary>
        /// <param name="dbScope">Объект, предоставляющий доступ к базе данных.</param>
        /// <param name="satelliteID">Идентификатор карточки-сателлита.</param>
        /// <param name="cancellationToken">Объект, посредством которого можно отменить асинхронную задачу.</param>
        /// <returns>
        /// Идентификатор основной карточки по идентификатору карточки-сателлита
        /// или <c>null</c>, если сателлит не найден.
        /// </returns>
        public static Task<Guid?> TryGetMainCardIDFromSatelliteIDAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            return CardSatelliteHelper.TryGetMainCardIDFromSatelliteIDAsync(
                dbScope,
                satelliteID,
                TaskSatelliteSection,
                TaskSatelliteMainCardIDField,
        ...
        /// или <c>null</c>, если сателлит не найден. Возвращает признак того, что карточка запрошенная информация найдена.
        /// Вторым значением возвращает идентификатор основной карточки или <c>Guid.Empty</c>, если карточка-сателлит не найдена.
        /// Третьим значением возвращает идентификатор задания или <c>Guid.Empty</c>, если карточка-сателлит не найдена.
        /// </summary>
        /// <param name="dbScope">Объект, предоставляющий доступ к базе данных.</param>
        /// <param name="satelliteID">Идентификатор карточки-сателлита.</param>
        /// <param name="cancellationToken">Объект, посредством которого можно отменить асинхронную задачу.</param>
        /// <returns>
        /// <c>true</c>, если запрошенная информация найдена;
        /// <c>false</c> в противном случае.
        /// </returns>
        public static Task<(bool result, Guid mainCardID, Guid taskRowID)> TryGetMainCardIDAndTaskRowIDAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            return CardSatelliteHelper.TryGetMainCardIDAndTaskRowIDAsync(
                dbScope,
                satelliteID,
                TaskSatelliteSection,
                TaskSatelliteMainCardIDField,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            using (DbDataReader reader = await db.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false))
            {
                while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
                {
                    ...
                    if (rowID.HasValue && historyItemsByRowID.TryGetValue(rowID.Value, out CardTaskHistoryItem item))
                    {
                        ...
                        if (loadCalendarInfo)
                        {
                            if (rowIDListToLoadCalendar is null)
                            {
            ...
            if (rowIDListToLoadCalendar != null)
            {
                ...
                db
                    .SetCommand(
                        builderFactory
                            ...
                            .As("OverdueQuants")
                            .C("cqPlanned", "QuantNumber").Substract().C("cqNow", "QuantNumber")
                            .As("UntilCompletionQuants")
                            .C("cqNow", "QuantNumber").Substract().C("cqInProgress", "QuantNumber")
                            .As("InProgressQuants")
                            ...
                            .CrossJoin(b => b
                                    ...
                                    .Limit(1),
                                "cqNow")
                            .Where().C("th", "RowID").In(rowIDListToLoadCalendar.ToArray())
                            .Build(),
        ...
        /// Созданную роль необходимо сохранить средствами <see cref="IRoleRepository"/>, прежде чем использовать.
        /// </summary>
        /// <param name="cardID">
        ...
        /// </remarks>
        public static async ValueTask<(bool result, TaskRole taskRole)> TryCreateResolutionPerformerRoleAsync(
            Guid cardID,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            await using (dbScope.Create())
            {
                ...
                foreach ((Guid roleID, string roleName) in performerRoles)
                {
                    if (!await FillUsersAsync(db, builderFactory, roleRepository, validationResult, users, roleID, roleName, cancellationToken).ConfigureAwait(false))
                    {
                        return (false, null);
                    }
                }
            ...
            IRoleUser[] distinctUsers = users.Distinct(RoleUserIDComparer.Instance).ToArray();
            TaskRole taskRole = RoleHelper.CreateTaskRole(Guid.NewGuid(), distinctUsers);

            taskRole.Name = taskRoleName;
            return (true, taskRole);
        }


        private static async Task<bool> FillUsersAsync(
            DbManager db,
            IQueryBuilderFactory builderFactory,
            IRoleRepository roleRepository,
            IValidationResultBuilder validationResult,
            List<RoleUserRecord> users,
            Guid roleID,
            string roleName,
            CancellationToken cancellationToken = default)
        {
            ...
            {
                validationResult.AddError(typeof(WfHelper), "$WfResolution_Error_PerformerIsNotPersonalRole", roleName);
                return false;
            }

            List<RoleUserRecord> durableUsers = await roleRepository.GetUsersAsync(roleID, cancellationToken).ConfigureAwait(false);
