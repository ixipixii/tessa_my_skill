...
namespace Tessa.Extensions.Default.Shared.Settings
{
    public class PartnersCardMetadataExtension : CardTypeMetadataExtension
    {
        ...
        {
            Card settingsCard = await this.clientCache.Cards.GetAsync(DefaultCardTypes.KrSettingsTypeName, cancellationToken).ConfigureAwait(false);
            if (settingsCard != null)
            {
                return settingsCard.Sections["KrSettings"].RawFields.Get<bool>("AllowManualInputAndAutoCreatePartners");
            }
        ...
        {
            foreach (var block in cardType.Blocks)
            {
