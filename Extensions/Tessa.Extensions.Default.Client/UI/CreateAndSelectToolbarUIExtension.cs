...
namespace Tessa.Extensions.Default.Client.UI
{
    public sealed class CreateAndSelectToolbarUIExtension : CardUIExtension
    {
        public override async Task Initialized(ICardUIExtensionContext context)
        {
            ...
            {
                var saveAndCloseItemIndex = actions.IndexOf(p => p.Name == TileNames.SaveAndCloseCard);
                if (saveAndCloseItemIndex != -1)
                {
                    var item = actions[saveAndCloseItemIndex];
                    actions.RemoveInputBinding(item, TileKeys.SaveAndCloseCard);
                    actions.RemoveAt(saveAndCloseItemIndex);
                }
                var saveCloseAndCreateItemIndex = actions.IndexOf(p => p.Name == TileNames.SaveCloseAndCreateCard);
                if (saveCloseAndCreateItemIndex != -1)
                {
                    var item = actions[saveCloseAndCreateItemIndex];
                    actions.RemoveInputBinding(item, TileKeys.SaveCloseAndCreateCard2);
                    actions.RemoveAt(saveCloseAndCreateItemIndex);
                }
                ...
                actions.AddOrReplaceInputBinding(saveAndCloseItem, TileKeys.SaveAndCloseCard);
            }
        }
