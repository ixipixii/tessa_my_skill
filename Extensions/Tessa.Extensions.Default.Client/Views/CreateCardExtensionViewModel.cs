        ...
        BaseContentItem
    {
        ...
        private async void CreateCardActionBySelectedRowAsync(object parameter)
        {
            ...
            if (!this.canCreateCard
                ...
                || (reference = view.Metadata.References.FirstOrDefault(x => x.IsCard && x.OpenOnDoubleClick)) == null)
            {
                return;
            }
        ...
        private async void CreateCardActionByDocTypeIdentifierAsync(object o)
        {
            ...
            if (!Guid.TryParse(this.settings.DocTypeIdentifier, out Guid docTypeID)
                || (docType = (await this.krTypesCache.GetDocTypesAsync()).FirstOrDefault(x => x.ID == docTypeID)) == null)
            {
                return;
            }
            ...
            Dictionary<string, object> info = null)
        {
            ...
            await using (UIContext.Create(context))
            {
                ...
                var idParamMeta = this.component.View?.Metadata.Parameters.FindByName(idParam);
                ...
                if (inSelectionMode || this.settings.CardOpeningKind == CardOpeningKind.ModalDialog)
                {
                    if (inSelectionMode
                        ...
                        && idObj is Guid id)
                    {
                        var request = new TessaViewRequest(this.component.View.Metadata);
                            ...
                            && result.Columns != null)
                        {
                            ...
                            this.component.Workplace.DoubleClickAction?.DoubleClick(
                                ...
                                {
                                    View = this.component.View.Metadata,
                                    Context = context,
                                    Sender = this,
                                    SelectedObject = row,
                                });
